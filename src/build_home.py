...
(function(){
  ...
  /* ===== VONG LAP LUON SONG =====
     Ban truoc dung mot cai chot `running` de chi chay rAF khi can. No hong theo
     kieu khong the tu hoi: neu trang duoc tai luc tab dang an (hoac truoc lan ve
     dau tien), `running` bat len true nhung rAF KHONG chay de tat lai. Tu do moi
     lan cuon deu thay running===true nen khong len lich rAF moi -> nen chet han
     cho toi khi tai lai trang.
     Gio giu mot vong rAF chay lien tuc. Khi khong co viec gi no thoat ra ngay,
     ton dung mot loi goi ham moi khung — doi lay viec khong con trang thai nao
     co the ket. */
  let shown=0,target=0,seeking=false,seekAt=0,navOn=false;
  ...
  function seek(){
    ...
    if(f===shownFrame) return;                       /* khung khong doi -> khong tua */
    const now=performance.now();
    if(now-lastSeek<SEEK_MIN_MS) return;             /* san toc do khi cuon rat nhanh */
    const t=f*FRAME;
    /* Gan currentTime dung bang gia tri hien tai thi KHONG sinh su kien 'seeked',
       ma co 'seeking' da bat -> ket vinh vien. */
  ...
  function pump(){
    requestAnimationFrame(pump);                     /* re-arm TRUOC moi viec khac */
    if(!v.duration) return;
    if(seeking && performance.now()-seekAt>500) seeking=false;   /* chong ket */
    const d=target-shown;
    if(Math.abs(d)<FRAME*0.25) shown=target;
    /* keo thanh cuon nhay mot phat thi nhay thang, khong noi suy, neu khong video
       se "boi" theo sau mot doan dai */
    else shown += Math.abs(d)>2.5 ? d*0.70 : d*0.25;
    seek();
  }
  requestAnimationFrame(pump);
  /* Quay lai tab sau khi roi di: rAF da bi tam dung, dong bo lai ngay */
  ...
  /* scrollHeight ep trinh duyet TINH LAI BO CUC ngay tai cho. Goi no trong ham
     cuon la ep tinh bo cuc dong bo hang chuc lan moi giay. Do mot lan, chi do lai
     khi kich thuoc that su doi. */
  let maxScroll=1;
  ...
  function onScroll(){
    ...
    reveal();
    /* Lam tron ty le phong to thanh 40 nac: de nguyen 4 chu so thap phan thi gan
       nhu moi lan cuon deu ra gia tri moi, ep gop lai ca lop video 1600px. */
  ...
  window._hero={onScroll,giveUp,seek,pump,state:()=>({shown,target,seeking,shownFrame})};
